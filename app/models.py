...
class Requerimiento(db.Model):
    ...
    __table_args__ = (db.UniqueConstraint('proyecto_id', 'identificador', name='uq_req_id'),)
    ...
    relaciones_origen = db.relationship('Trazabilidad', foreign_keys='Trazabilidad.requerimiento_origen_id',
                                        backref='origen', lazy='dynamic', cascade='all, delete-orphan')
    relaciones_destino = db.relationship('Trazabilidad', foreign_keys='Trazabilidad.requerimiento_destino_id',
                                         backref='destino', lazy='dynamic', cascade='all, delete-orphan')

caso_uso_requerimiento = db.Table('caso_uso_requerimiento',
...
class CasoUso(db.Model):
    ...
    fecha_creacion = db.Column(db.DateTime, default=datetime.utcnow)
    __table_args__ = (db.UniqueConstraint('proyecto_id', 'identificador', name='uq_cu_id'),)
    requerimientos = db.relationship('Requerimiento', secondary=caso_uso_requerimiento,
                                     backref=db.backref('casos_uso', lazy='dynamic'), lazy='dynamic')
...
class Trazabilidad(db.Model):
    ...
    fecha_creacion = db.Column(db.DateTime, default=datetime.utcnow)
    __table_args__ = (db.UniqueConstraint('requerimiento_origen_id', 'requerimiento_destino_id', name='uq_traz'),)
...
class Comentario(db.Model):
    ...
    autor = db.Column(db.String(80), default='Anónimo')
    fecha_creacion = db.Column(db.DateTime, default=datetime.utcnow)